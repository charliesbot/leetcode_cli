...
test('CLI integration test suite', async (t) => {
  ...
  await t.test('should list available languages when no language specified', async () => {
    ...
    assert(combinedOutput.includes('Available languages:') || combinedOutput.includes('Please specify a language'));
  ...
  await t.test('should handle invalid language gracefully', async () => {
    ...
    const combinedOutput = result.stdout + result.stderr;
    assert(combinedOutput.includes("Language 'invalidlang' not supported") || combinedOutput.includes('not supported'));
  ...
  await t.test('should initialize empty workspace successfully', async () => {
    ...
    const workspaceExists = await fs.access(join(testWorkspace, '.leetkick.json')).then(() => true).catch(() => false);
  ...
  await t.test('should add language to workspace successfully', async () => {
    ...
    const typescriptExists = await fs.access(join(testWorkspace, 'typescript')).then(() => true).catch(() => false);
